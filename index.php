...
if (isset($_POST) && trim($_POST['product_txt']) != '') {
    
    $products = trim($_POST['product_txt']);
   
    $terminal = new Terminal();    
    $terminal->set_pricing("A", 2, 4, 7);
    $terminal->set_pricing("B", 12);
    $terminal->set_pricing("C", 1.25, 6, 6);
    $terminal->set_pricing("D", 0.15);
     
    $terminal->scan($products);
    
    print "Scan these items in this order: {$products}; Verify the total price is $".$terminal->total();
